...
async function persistCheckin(svc, userId, checkin) {
  try {
    ...
    const notes = [
      'Check-in mensual:',
      checkin.feel ? `se sintió ${FEEL_ES[checkin.feel] || checkin.feel}` : null,
      checkin.adherence ? `cumplió ${DONE_ES[checkin.adherence] || checkin.adherence}` : null,
...
export async function generatePlan(kind, req, res) {
  ...
  const built = kind === 'diet' ? buildDietMessages(profileForPrompt, ctx) : buildWorkoutMessages(profileForPrompt, ctx)
  ...
  try {
    ...
    if (checkin) await persistCheckin(svc, user.id, checkin)
